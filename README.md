...
**what did this earn me?** It does that in three acts.

| Act | Panel | The claim |
| --- | --- | --- |
| **Act** | Cross-property action items | A review tool is only useful if its output is a task you can assign. Every row is a work order — *"re-caulk the master shower and swap the low-flow head"* — never *"guests mentioned the bathroom"*. |
| **Prove** | Effectiveness & pricing tracker | Mark a fix implemented with a date; the rating chart splits into before/after at that date. Once a lift holds across enough reviews, it becomes a directional pricing suggestion. |
| **Compare** | Benchmarking | Where the property sits inside the portfolio, where it sits in the local market, and the one category where anonymised local top performers still beat it. |

Selecting an action item in **Act** re-scopes **Prove** and **Compare**, so all
three panels always describe the same decision.
...
Requires Node 18+, which is **not currently installed on this machine**. Install
it first (`brew install node`, or via [nvm](https://github.com/nvm-sh/nvm)), then:
